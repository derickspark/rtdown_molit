...
.empty_result <- function(type, lawd_cd = NA_character_,
...
# 응답 row 리스트 -> tibble
.rows_to_tibble <- function(rows, type, lawd_cd, deal_ymd,
                            sigungu_nm = NA_character_) {
  if (length(rows) == 0L) {
    return(.empty_result(type, lawd_cd, sigungu_nm, deal_ymd))
  }
  n <- length(rows)
  schema <- .get_schema(type)
  cols <- list(
